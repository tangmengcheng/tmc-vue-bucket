...
现在我们数据劫持完了，那我们要做一个初渲染是吧？
那肯定是要把当前数据渲染到我们的页面上，那么就需要进入下一个流程：如果用户传了 el ，就要去执行挂载流程了 ===》渲染模板了

模板的哪几种：template、div、render

--- 默认先会查找 render -> template -> el 中的内容代替

如果用户传了 el 属性，需要将页面渲染出来
